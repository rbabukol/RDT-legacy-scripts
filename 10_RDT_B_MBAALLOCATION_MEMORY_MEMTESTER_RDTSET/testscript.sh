...
pqos -R > /dev/null
echo " ------------------------------ "
pqos -s | grep "MBA COS" > mba_before.txt
echo "Set MBA to 50% for cores 5,6 and start memtester on those cores"
rdtset -t "mba=50;cpu=5-6" -c 5-6 memtester 10M &> /dev/null &
sleep 5s
pqos -s | grep "MBA COS" > mba_after.txt
...
if [ $(($FOUND1 + $FOUND2)) -ne 2 ]; then
	echo "Error - could not confirm pqos changes"
	exit 1
fi
...
rdtset -t "mba=200;cpu=5-6" -c 5-6 memtester 10M &> 200_temp.txt 
sleep 5s
