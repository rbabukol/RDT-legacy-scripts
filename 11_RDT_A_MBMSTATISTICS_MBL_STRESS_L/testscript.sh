...
echo " ------------------------------"
TARGET_CORE=0
echo "Start memory load tool with simultaneous redirection of the process to one selected CPU core ($TARGET_CORE)"
taskset -c $TARGET_CORE stress -m 100 &> /dev/null &
echo "Sleep 5s"
sleep 5s
echo " ------------------------------"
SAMPLE_TIME=60
echo "Start collecting MBL statistics for physical core $TARGET_CORE with pqos for $SAMPLE_TIME s"
rm -fr mbl.txt
pqos --mon-core="mbl:$TARGET_CORE" --mon-file=mbl.txt -t $SAMPLE_TIME &> /dev/null
echo "Terminate stress"
killall stress
echo " ------------------------------ "
echo "Check MBL in RDT log to confirm successful logging..."
grep -E " $TARGET_CORE  [0-9].[0-9]{2} " mbl.txt > temp2
